...
def main() -> None:
    ...
    only_capitalized = [player for player
                        in player_list if player == player.capitalize()]
    print(f"New list of capitalized names only: {only_capitalized}")
    score_dict = {player: random.randint(0, 1000)
                  for player in all_capitalized}
    print(f"Score dict: {score_dict}")
    scores = list(score_dict.values())
    average_score = sum(scores) / len(scores)
    print(f"Average score is {round(average_score, 2)}")
    only_high_scores = {key: value for key, value
                        in score_dict.items() if value > average_score}
